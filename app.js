...
io.sockets.on('connection', function(socket) {
    ...
    socket.on('join', function(data) {
        if(gameStart) {
            socket.emit('joinResponse', {success:false,msg:'Ha comenzado.'});
        } else if(data.username.length >= 10) {
            socket.emit('joinResponse', {success:false,msg:'Muy largo (<10).'});
        } else if(getCurrentPlayers().length == maxPlayerNumber) {
            socket.emit('joinResponse', {success:false,msg:'Jugador máximo #6.'});
        } else if(!checkValidCharacter(data.username)) {
            socket.emit('joinResponse', {success:false,msg:'No puede contener caracteres especiales (<,>,\\,/,etc).'});
            ...
            socket.emit('joinResponse', {success:false,msg:'Ya está ocupado.'});
        }
    });
    ...
    socket.on('ready', function(data) {
        ...
        // Only one player
        if(players.length === 1) {
            sendMessage(SOCKET_LIST[socket.id], 'Espera a otros jugadores.');
            return;
        }
...
function chooseWord() {
    ...
    for(var i in SOCKET_LIST) {
        ...
        if(SOCKET_LIST[i].id != currentPlayer.id) {
            sendMessage(SOCKET_LIST[i], currentPlayer.username + ' está escogiendo palabra...');
        }
    }
}
...
function gameOver() {
    ...
    for(var i in inGamePlayers) {
        ...
        SOCKET_LIST[inGamePlayers[i].id].emit('gameOver', {msg:'Gracias por jugar!',winner:players});
        sendMessage(SOCKET_LIST[inGamePlayers[i].id], '===================');
    }
    resetGame();
}
