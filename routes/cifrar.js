...
router.post(
  ...
  [
    ...
    body(
      ...
      .matches(/^[a-zA-Z0-9]+(\s*[a-zA-Z0-9]*)*[a-zA-Z0-9]+$/),
  ],
    ...
    try {
      if (valores.type === '192') {
        ...
        if (key.length === 16 || key.length === 32) {
          var error = 'No puede cifrar en AES 192';
      ...
      if (valores.type === '256') {
        ...
        if (key.length === 16 || key.length === 24) {
          var error = 'No se puede cifrar en AES 256';
      ...
      if (valores.type === 'bcrypt') {
        ...
        encrypted = await bycrypt.hash(valores.nya, salt);

        fs.writeFile(
          `./files/encrypted/BCRYPT-${Date.now()}1.txt`,
          encrypted,
          (err) => {
            if (err) {
              return console.error(err);
        ...
        var filename = `BCRYPT-${Date.now()}1.txt`;
      ...
      if (valores.type === '128') {
        ...
        if (key.length === 24 || key.length === 32) {
          var error = 'No se puede cifrar en AES 128';
